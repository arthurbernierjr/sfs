...
- Make this a git repo by running `git init` within the root of `project-1-assessment`, make sure you use pwd to make sure you are directly in the project-1-assessment folder
- create a git repo on github.com
- copy the ssh url and run `git remote add origin sshlinkhere`
- then run `code .`
- If you `git init` in the wrong folder you can reverse the process by first running `ls -a` to verify there is a `.git` folder, then running `rm -rf .git` to remove the `.git folder` permananently.
...
**Push your completed code to github when your done**
