...
const myShip = new Ship('USS Schwarzenegger', 20, 5, 0.7)
...
const createEnemies = () => {
  for (let i = 0; i < 6; i++) {
    ...
    const accuracy = randomNum(0.6, 0.8)
    console.log(hull, firepower, accuracy)
    const enemy = new EnemyShip(hull, firepower, accuracy)
    console.log(enemy)
...
const promptBattle = () => {
  ...
  const playerChoice = prompt('Should we engage?', '(e)ngage || (r)etreat')
  if (playerChoice === 'r') {
    myShip.retreat()
