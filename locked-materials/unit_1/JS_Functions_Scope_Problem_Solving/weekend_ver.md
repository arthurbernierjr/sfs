...
1. In tonight's `homework` folder, create a file called `solutions.js`. Write your answers in that file

## Verbal Questions 

Write answers to the following questions as comments. As a reminder you can highlight your code and type `command` + `/` to turn your code into a comment, like so:
...
> Note: A palindrome is a word that reads the same way backwards and forwards.
...
_hint:_ forgot what the pythagorean theorem was? Do a quick google search to find the equation! 
...
To test whether a number is Prime, you only need to check if the number is not evenly divisible by any number from 2 up to the **square root** of that number. Checking only up to the square root is advisable for optimization and testing large numbers.
  - If it is not evenly divisble by any number between that range, it is prime.
  - If it is evenly divisible by any number between that range, it is not prime. 
...
***Revisit Question From Morning Lab***
...
- Go back and do any lab problems you weren't able to complete!
